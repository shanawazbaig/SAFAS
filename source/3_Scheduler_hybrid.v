...
//                  All Rights Reserved
//
// Description:     Hybrid EDF + SRJF Scheduler with Slack Threshold
...
///////////////////////////////////////////////
`timescale 1ns / 1ns
module Scheduler_hybrid #(parameter W = 42, R_Q = 64, CORE = 16)(
   input clk, rst,
   input CTRL_action,
   input CTRL_subtract,
   input CTRL_RP,
...
Insertion_block_hybrid #(.W(W), .N(R_Q), .MODE(0)) SRJF_Q(
   //INPUT
   .clk       (clk), //PI
   .rst       (rst), //PI
...
/************ EDF-Priority queue *************/
// This queue uses EDF for critical/urgent tasks (slack < threshold)
Insertion_block_hybrid #(.W(W), .N(64), .MODE(1)) EDF_Q(
   //INPUT
   .clk       (clk), //PI
   .rst       (rst), //PI
   .subtract  (CTRL_subtract), //PI
...
/************* Situation 2 - Hybrid EDF+SRJF **************/
   // Choose between EDF queue and SRJF queue based on slack
   // If EDF queue has tasks with low slack (urgent), prioritize them
...
/********** statistical data ************/
   ...
   always @(posedge clk) begin
        ...
        if(check_s1 & CTRL_action & (~empty|~empty_critical)) begin
            RP_Run = RP_Run+1;
            if(check_critical) RP_EDF_scheduled = RP_EDF_scheduled + 1;
