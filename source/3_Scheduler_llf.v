...
//                  All Rights Reserved
//
// Description:     LLF Hybrid Scheduler with Laxity Threshold
//                  - Uses SRJF (Shortest Remaining Job First) normally
//                  - When laxity < threshold (1.5× remaining execution time), switches to LLF mode
//                  - Laxity = (Relative_deadline - Execution_time)
//                  - Threshold = 1.5 × Execution_time
...
///////////////////////////////////////////////
`timescale 1ns / 1ns
module Scheduler_llf #(parameter W = 42, R_Q = 64, CORE = 16)(
   input clk, rst,
   input CTRL_action,
   input CTRL_subtract,
   input CTRL_RP,
...
/************* SRJF Queue **************/
// This queue uses SRJF (Shortest Remaining Job First) ordering
// For tasks with high laxity (laxity >= 1.5× execution)
Insertion_block_llf #(.W(W), .N(R_Q), .MODE(0)) SRJF_Q(
   //INPUT
   .clk       (clk), //PI
   .rst       (rst), //PI
...
/************ LLF-Priority queue *************/
// This queue uses LLF for critical/urgent tasks (laxity < threshold)
// Sorted by laxity (lower laxity = higher priority)
Insertion_block_llf #(.W(W), .N(64), .MODE(1)) LLF_Q(
   //INPUT
   .clk       (clk), //PI
   .rst       (rst), //PI
   .subtract  (CTRL_subtract), //PI
...
/************* Situation 2 - LLF Hybrid **************/
   // Choose between LLF queue and SRJF queue based on laxity
   // If LLF queue has tasks with low laxity (urgent), prioritize them
   // Calculate laxity for comparison
   wire [15:0] laxity_critical = task_critical_out[31:16] - task_critical_out[15:0];
   wire [15:0] laxity_normal = task_out[31:16] - task_out[15:0];

   assign check_critical =(~empty_critical)?((~empty)? laxity_critical <= laxity_normal : 1'b1) : 1'b0;
   wire [15:0] tmp_s2    =(check_critical)? laxity_critical : laxity_normal;

   // Calculate laxity of running task for comparison
   wire [15:0] running_laxity = RT_before_S[s2_index][31:16] - RT_before_S[s2_index][15:0];

   wire find_s2          =((RT_before_S[s2_index][40])?
                             tmp_s2 < running_laxity
                            :tmp_s2 < (RT_before_S[s2_index][31:16] - RT_before_S[s2_index][15:0])) & (~empty | ~empty_critical) & CTRL_action;
   assign check_s2       = find_s2 & ~s1_valid & ~wr_preempt;
...
/********** statistical data ************/
   ...
   integer RP_Run       = 0;
   integer RP_SRJF_scheduled = 0;
   integer RP_LLF_scheduled = 0;
   ...
   always @(posedge clk) begin
        ...
        if(check_s1 & CTRL_action & (~empty|~empty_critical)) begin
            RP_Run = RP_Run+1;
            if(check_critical) RP_LLF_scheduled = RP_LLF_scheduled + 1;
            else RP_SRJF_scheduled = RP_SRJF_scheduled + 1;
        end
   end
